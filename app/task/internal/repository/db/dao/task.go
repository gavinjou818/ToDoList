...
func (dao *TaskDao) ListTaskByUserId(userId int64) (r []*model.Task, err error) {
	err = dao.Model(&model.Task{}).Where("user_id=?", userId).Find(&r).Error
	return
}
...
func (dao *TaskDao) DeleteTaskById(taskId, userId int64) (err error) {
	err = dao.Model(&model.Task{}).Where("task_id =? AND user_id = ?", taskId, userId).Delete(model.Task{}).Error
	return
}
...
func (dao *TaskDao) UpdateTask(req *taskPb.TaskRequest) (err error) {
	...
	err = dao.Model(&model.Task{}).Where("task_id=?", req.TaskID).Updates(&taskUpdateMap).Error
	return
}
